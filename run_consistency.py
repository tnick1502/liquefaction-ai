...
# 2) Vs ↔ G0 (digitrock) — самосогласованность по сохранённой схеме артефакта.
# G0 как отдельная колонка не персистится; реконструируем модуль сдвига из сохранённых Vs и
# плотности по обратной формуле digitrock G0 = ρ·Vs²/1000 (МПа) и проверяем замкнутость
# Vs = sqrt(G0·1000/ρ) плюс физический диапазон Vs и G0.
rho = m["r"].to_numpy(float)
vs_arr = m["V_s"].to_numpy(float)
g0_recon = rho * vs_arr ** 2 / 1000.0
vs_chk = vs_from_g0(g0_recon, rho)
vs_err = np.nanmax(np.abs(vs_chk - vs_arr))
print(ok(vs_err < 1.0), f"Vs == sqrt(G0·1000/ρ) (digitrock РК, G0=ρ·Vs²/1000), max|Δ|={vs_err:.3f} м/с")
vs = m["V_s"].astype(float)
print(ok((vs.min()>=40) and (vs.max()<600)), f"Vs в физ. диапазоне: {vs.min():.0f}–{vs.max():.0f} м/с (median {vs.median():.0f})")
print(ok(np.isfinite(g0_recon).all() and (g0_recon>0).all()),
      f"G0 определён для всех (нет пропусков), G0 median={np.median(g0_recon):.1f} МПа")

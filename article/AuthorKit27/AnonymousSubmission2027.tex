...
20 sites}; the strict landmark benchmark contains \textbf{812 tests across 14 sites} after excluding both
...
physically constrained latent parameters $\theta$ with a \emph{prefix-conditioned} conditional flow, and
integrates a differentiable analytical CRR/damage/PPR layer; uncertainty is propagated through the flow
by Monte-Carlo sampling at inference. (The reported model normalizes a proper flow density over
$\theta$; optional test-time gradient refinement of $\theta$ is evaluated only as an ablation, since its
Jacobian is not part of the normalized density.) Our contributions are: (i) a \textbf{prefix-conditioned
...
(iv) an \textbf{object-held-out landmark benchmark} on 812 tests from a 1{,}093-test archive with strong tabular, sequence and
...
risk set contains 812 tests across 14 sites (490 liquefied, 322 non-liquefying); 137 events and 144 censored
tests occur before the landmark and are excluded. The by-3000 risk label is known for 490 positives and

...
constrained parameters through a \emph{conditional affine flow} and rolls them through a
\emph{differentiable analytical CRR/damage/PPR layer}, guaranteeing feasible monotone accumulation;
a censoring-aware objective trains jointly on liquefied, stabilized, and unfinished tests, and a
Monte-Carlo predictive propagates uncertainty through the flow. On \textbf{1{,}093 real cyclic tests
across 20 sites}, under \emph{object-held-out} repeated cross-validation with object-cluster bootstrap
confidence intervals, DPI-Flow provides the best \emph{physically admissible} Pareto balance of onset
accuracy, post-prefix trajectory quality, calibrated uncertainty, and structural feasibility: it leads
the admissible $\mathrm{P}^3$ ranking, attains the lowest worst-state trajectory error and the lowest
trajectory CRPS among admissible models, near-zero physics violations, and recovers a measured
cyclic-resistance curve. Strong unconstrained baselines (CatBoost on scalar onset; a Transformer on raw
trajectory) win individual metrics but violate physics or omit uncertainty/CRR. We release a reproducible
object-held-out benchmark and ablations isolating each component.
\end{abstract}
...
objective} that learns from liquefied (exact), stabilized (right-censored) and unfinished tests; (iv) an
\textbf{object-held-out benchmark} on 1{,}093 real tests with strong tabular, sequence and
...
\paragraph{Three censoring regimes.} An observability mask $o_i\in\{0,1\}$ encodes: (1)~liquefied
($y_i{=}1$): $N_{\mathrm{liq},i}$ observed exactly, $o_i{=}1$; (2)~stabilized non-liquefying (flat PPR
tail past a practical horizon): $N_{\mathrm{liq},i}$ right-censored, $o_i{=}1$; (3)~unfinished
non-liquefying: terminal unobservable, $o_i{=}0$---excluded from $N_{\mathrm{liq}}$ supervision/metrics but
retained for trajectory learning \citep{cox72,deepsurv18}.
...
through a \emph{conditional affine flow} $T_\phi$ \citep{rezende15,realnvp17}:
\begin{equation}
z_i=\mu(h_i)+\sigma(h_i)\odot\epsilon,\quad \epsilon\sim\mathcal{N}(0,I),\quad \theta_i=T_\phi(z_i;h_i),
\end{equation}
where $\log\sigma^2(h_i)$ is clipped to $[-5,3]$ and each of the two flow blocks applies
$u\mapsto u\odot e^{\,0.35\tanh(W_s h_i)}+W_t h_i$. A bounding map $\pi_i=\Theta(\theta_i)$ sends each raw
...
monotone-accumulation assumption. The event time is a differentiable soft first-hitting of the monotone
envelope $\bar r=\mathrm{cummax}(\hat r)$ with $p_t=\sigma\!\big(\beta_0(\bar r_t-\tau)\big)$,
...
(with conformal scale $s$, below), the loss is
\begin{align}
...
\end{equation}
i.e.\ exact for liquefied, one-sided for right-censored stabilized specimens, and masked ($o_i{=}0$) for
unfinished ones \citep{cox72,dsm21}. $\mathcal{L}_{\mathrm{reg}}$ collects label-free physics
regularizers (CRR monotonicity, boundedness, second-order smoothness).

\paragraph{Conformal calibration.} A single scale $s$ is fit on the \emph{validation set of each outer
fold} so that the empirical $90\%$ interval coverage matches nominal, then applied as
$\hat v\leftarrow\hat v\,e^{2s}$ \citep{guo17,romano19}.
...
\paragraph{Data.} 1{,}093 real cyclic tests across 20 sites (640 liquefied, 453 non-liquefying), with a
measured CRR curve available for a subset of sites; Vs is derived from small-strain $G_0$, grain-size and
...
CatBoost            & \textit{0.996} & \textit{0.038} & 0.051 & -- & -- & -- & -- & \textit{0.214} & -- & -- \\
\bottomrule
\end{tabular}
\end{table*}
...
\emph{individual} metrics: CatBoost attains the best scalar onset (AUPRC $0.996$, Brier $0.038$) and the
best $N_{\mathrm{liq}}$ log-MAE ($0.214$), but produces no trajectory, no calibrated uncertainty and no

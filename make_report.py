...
P("DPI-EVT — объединённая модель (идентификация параметров + событийный движок + CRR-физика)", "H3")
P("Третья, объединяющая архитектура: <b>DPI работает как модуль идентификации параметров</b> "
  "(энкодер по грунту и префиксу PPR → апостериор θ + нормализующий поток), а <b>EVT — как движок</b>, "
  "разворачивающий из θ единое латентное физическое состояние: повреждение Damage(N), поровое "
  "давление ru(N)=PPR(N), триггер и N<sub>liq</sub>. Ключевая особенность — <b>дифференцируемая "
  "CRR-физика</b>: кривая сопротивления CRR(N) не подгоняется эмпирически, а выводится как следствие "
  "закона повреждения, CRR(N)=CRR<sub>ref</sub>·(1+&lambda;N)<sup>&minus;1/m</sup> (та же θ, что "
  "управляет Damage). В результате PPR, Damage, CRR, ru и N<sub>liq</sub> предсказываются из одного "
  "когерентного физического состояния: N<sub>liq</sub> берётся из момента пересечения порога самой "
  "кривой PPR, а joint-consistency-связи (CRR(N<sub>liq</sub>)≈CSR, Damage(N<sub>liq</sub>)≈порог) "
  "согласуют выходы. С разделением сопротивления (<i>decoupled</i>: гибкое — для динамики, физический "
  "степенной закон — для отчётной CRR) и малым резидуалом траектории это делает DPI-EVT <b>наиболее "
  "сбалансированной моделью исследования</b>: лучший CRPS (0.048) и N<sub>liq</sub> (log-MAE 0.45), "
  "вторая по PPR RMSE (0.098), лучшая граница CRR (0.123), AUROC 1.0 и нулевые физические нарушения.")

...
def author_hyperbolic_csr(
    ...
    """
    Пример использования модуля на синтетическом диапазоне циклов.

    :return: tuple из:
             - массива числа циклов
             - словаря с рассчитанными кривыми CSR(N)
    """
    n_cycles: np.ndarray = np.logspace(0, 4, 300)

    result: Dict[str, np.ndarray] = compute_all_csr_models(
        n_cycles=n_cycles,
        bilge_params={"alpha": 5.0, "beta": -8.0},
        lentini_params={"alpha": -0.03, "beta": 0.35},
        guoxing_params={"alpha": 1000.0, "beta": -2.5},
        meziane_params={"alpha": 2.4, "beta": 9.5},
        hyperbolic_params={"alpha": 0.2, "beta": 0.4},
    )

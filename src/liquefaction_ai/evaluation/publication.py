...
def pareto_plot(summary_df: pd.DataFrame, x: str = "Traj_RMSE_continuation", y: str = "AUPRC",
    ...
    gate = 0.0 # strict P3 feasibility: every forecast curve must satisfy the declared contract
    fig, ax = plt.subplots(figsize=(6.2, 5.0))
    for _, r in d.iterrows():
        m = r["model"]; admissible = float(pvr.get(r.name, 0.0)) <= gate + 1e-12
    ...
    ax.scatter([], [], s=90, c=BLUE, edgecolor=INK, label="physics-admissible (PVR=0)")

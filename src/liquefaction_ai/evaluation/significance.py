...
def wilcoxon_signed_rank(d: np.ndarray):
    """Двусторонний Wilcoxon signed-rank (нормальная аппроксимация, поправка на непрерывность)."""
...
def object_cluster_bootstrap(df: pd.DataFrame, ref: str = "DPI-Flow", nboot: int = 2000,
    ...
    if "object" not in df.columns:
        raise ValueError("нужна колонка 'object' (кластер) для object-cluster bootstrap")
    models = list(df["model"].unique())
    rng = np.random.default_rng(seed)
    objects = np.array(sorted(df["object"].unique()))
    ...
    idx_by_obj = {m: {o: by[m].index[by[m]["object"].to_numpy() == o].to_numpy() for o in objects} for m in models}

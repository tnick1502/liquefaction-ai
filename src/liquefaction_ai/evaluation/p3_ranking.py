...
- **Физическая допустимость** — жёсткий gate + мягкий штраф: модель, выдающая физически невозможные
  кривые (Physics_Violation_Rate выше порога), не может быть лучшей публикационной моделью.
...
def compute_physical_admissibility(physics_violation_rate: float, soft_threshold: float = 0.01,
    ...
    Логика:
    - PVR ≤ soft_threshold → штраф 0, gate 1, надёжна;
    - soft < PVR ≤ hard → мягкий штраф = penalty_strength·max(0, PVR−soft)/(hard−soft), gate 1, надёжна;
    - PVR > hard → физически ненадёжна, gate 0;
...
def _physics_traj_col(df: pd.DataFrame) -> str:
    """Траекторная метрика для physics-режима: Traj_CRPS при наличии, иначе Traj_RMSE."""
...
def build_pareto_objectives(df: pd.DataFrame, mode: str = "core",
    ...
    out = df.copy()
    bases = [_physics_traj_col(df) if b == "__TRAJ__" else b for b in _P3_PARETO[mode]]
    obj_cols = []
    for base in bases:
